 ...
 */
import { ApiClientError, statusToErrorCode } from './errors.js';
...
export class ApiClient {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        if (attempt < attempts - 1) {
          ...
          await delay(backoff + Math.random() * RETRY_BASE_DELAY_MS, options.signal);
        }
      }
    }
  ...
  async #attempt<T>(options: RequestOptions<T>): Promise<T> {
