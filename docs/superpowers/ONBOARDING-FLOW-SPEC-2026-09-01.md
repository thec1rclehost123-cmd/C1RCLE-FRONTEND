...
or sign in" → wizard → pending review**. The V2 build has to keep that *shape* but run on the V2
...
| Firebase email OTP + phone OTP screens | **drop** both. Phone stays as a *typed field* on the profile (backend requires it), just not verified |
...
Whether a standalone `/signup` route still exists is Anil's call — this flow does signup *inside*
