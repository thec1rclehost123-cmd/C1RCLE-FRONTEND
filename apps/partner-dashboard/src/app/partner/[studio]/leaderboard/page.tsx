...
export default async function StudioLeaderboardPage({ params, searchParams }: { readonly params: Promise<{ studio: string }>; readonly searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const { studio } = await params;
  if (studio !== 'promoter') return renderStudioSkeleton(Promise.resolve({ studio }), 'Leaderboard', 'Leaderboard data is intentionally unavailable during the foundation checkpoint.');
  ...
  const period = data.periods.some((option) => option.value === periodValue) ? periodValue ?? 'all' : 'all';
  const city = data.cities.some((option) => option.value === cityValue) ? cityValue ?? 'global' : 'global';
