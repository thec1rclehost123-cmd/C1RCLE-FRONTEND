...
export default async function StudioDoorPage({ params }: { readonly params: Promise<{ studio: string }> }) {
  const { studio } = await params;
  if (studio !== 'venue' && studio !== 'host') return renderStudioSkeleton(Promise.resolve({ studio }), 'Door', 'Door Mode is available for Venue and Host in this checkpoint.');
