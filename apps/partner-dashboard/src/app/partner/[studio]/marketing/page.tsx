...
export default async function StudioMarketingPage({ params }: { readonly params: Promise<{ studio: string }> }) {
  const { studio } = await params;
  if (studio !== 'venue' && studio !== 'host') notFound();
  const data = studio === 'venue' ? await fixturePartnerDataSource.getVenueMarketing() : await fixturePartnerDataSource.getHostMarketing();
