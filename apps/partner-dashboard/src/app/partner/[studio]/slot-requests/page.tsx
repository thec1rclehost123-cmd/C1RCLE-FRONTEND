...
export default async function StudioSlotRequestsPage({
  params,
  searchParams,
}: {
  ...
  const view: SlotRequestScreenProps['initialView'] =
    getValue(query['status']) === 'all' ? 'all' : 'pending';
  const panel: SlotRequestScreenProps['initialPanel'] =
