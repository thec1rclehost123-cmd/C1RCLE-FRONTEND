...
export default async function StudioCreateEventPage({ params, searchParams }: { readonly params: Promise<{ studio: string }>; readonly searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <PartnerEventEditor data={data} availability={availability} mode="create" {...(step ? { initialStep: step } : {})} {...(venue ? { initialVenueId: venue } : {})} {...(date ? { initialDate: date } : {})} {...(slot ? { initialSlotId: slot } : {})} />;
}
