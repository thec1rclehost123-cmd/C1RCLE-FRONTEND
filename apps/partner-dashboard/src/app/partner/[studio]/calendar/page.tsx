...
export default async function StudioCalendarPage({ params, searchParams }: { readonly params: Promise<{ studio: string }>; readonly searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <HostAvailabilityScreen data={data} {...(month ? { initialMonth: month } : {})} {...(date ? { initialDate: date } : {})} {...(venue ? { initialVenue: venue } : {})} {...(slot ? { initialSlot: slot } : {})} />;
}
