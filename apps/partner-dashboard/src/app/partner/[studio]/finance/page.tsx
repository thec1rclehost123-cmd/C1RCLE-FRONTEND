...
export default async function StudioFinancePage({ params, searchParams }: { readonly params: Promise<{ studio: string }>; readonly searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const status: 'All' | FinanceOrderStatus = statusValue === 'confirmed' ? 'Confirmed' : statusValue === 'pending' ? 'Pending' : statusValue === 'refunded' ? 'Refunded' : statusValue === 'cancelled' ? 'Cancelled' : 'All';
  const sortValue = getValue(query['sort']);
  const sort: 'tickets' | 'amount' | null = sortValue === 'tickets' || sortValue === 'amount' ? sortValue : null;
  ...
  const props = { data, view, search: getValue(query['search']) ?? '', range, status, sort, direction };
