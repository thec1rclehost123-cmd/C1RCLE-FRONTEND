...
const chartPath = (values: readonly number[]): string => values.map((value, index) => {
...
const initials = (name: string): string => name.split(' ').map((word) => word[0]).join('').slice(0, 2);
...
export async function PromoterOverviewScreen() {
  ...
  const activity = [
    ...
    ...overview.calendar.filter((item) => item.type === 'deadline').slice(0, 2).map((item) => ({
      icon: 'link' as const,
      label: item.label,
      time: item.date,
  ...
  return (
    <div className={className('overview')}>
      <header className={className('pageHeader')}>
        <div><h1>Good evening, {overview.profile.name.split(' ')[0] ?? 'promoter'}</h1><p>Thursday, July 16</p></div>
      </header>

      <div className={className('topGrid')}>
        {nextEvent ? (
          <section className={`${className('panel')} ${className('tonight')}`} aria-labelledby="promoter-event-title">
            <div className={className('poster')} role="img" aria-label={`${nextEvent.name} artwork`}>
              <div className={className('posterCopy')} aria-hidden="true"><small>{nextEvent.date}</small><strong>{nextEvent.name.split(' ')[0]}<br />{nextEvent.name.split(' ').slice(1).join(' ')}</strong><span>{nextEvent.category}</span></div>
            </div>
            <div className={className('tonightBody')}>
              <div><span className={className('eyebrow')}>{nextEvent.status === 'active' ? 'Active' : 'Invitation'}</span><h2 id="promoter-event-title">{nextEvent.name}</h2><p className={className('venue')}><LocationIcon size={16} aria-hidden="true" />{nextEvent.venue} · {nextEvent.city}</p></div>
              <div className={className('tonightStats')}>
                <div className={className('doorTime')}><span>Event date</span><strong>{nextEvent.date}</strong><small>{nextEvent.time}</small></div>
                <div className={className('capacity')}><span>Tickets moved</span><div><strong>{nextEvent.tickets}</strong><small> tickets</small></div><small>{nextEvent.clicks.toLocaleString('en-IN')} clicks · {nextEvent.conversion}% conversion</small></div>
              </div>
              <Link href={`/promoter/events/${nextEvent.id}`} className={className('openEventButton')}>Open event</Link>
            </div>
          </section>
        ) : (
          <section className={`${className('panel')} ${className('tonight')} ${className('empty')}`}><div className={className('tonightBody')}><span className={className('eyebrow')}>No linked event</span><h2>Link an event to start tracking performance.</h2><Link href="/promoter/events?view=discover" className={className('openEventButton')}>Discover events</Link></div></section>
        )}

        <section className={`${className('panel')} ${className('activity')}`} aria-labelledby="promoter-activity-title">
          <div className={className('sectionHeading')}><h2 id="promoter-activity-title">Recent activity</h2><Link href="/promoter/events">View all</Link></div>
          <div className={className('activityList')}>
            {activity.map((item) => { const ActivityIcon = activityIcons[item.icon]; return <Link href={item.href} key={`${item.label}-${item.time}`} className={className('activityItem')}><span className={`${className('activityIcon')} ${className(item.icon === 'order' ? 'accent' : 'warning')}`}><ActivityIcon size={17} strokeWidth={1.8} aria-hidden="true" /></span><span className={className('activityCopy')}><strong><span>{item.label}</span><time>{item.time}</time></strong></span><NextIcon size={18} aria-hidden="true" /></Link>; })}
          </div>
        </section>
      </div>
