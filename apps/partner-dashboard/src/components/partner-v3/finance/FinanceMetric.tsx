...
export function FinanceMetric({ metric, icon }: { readonly metric: FinanceMetricData; readonly icon: ReactNode }) {
  ...
  return (
    <article className={styles['financeMetric']}>
      <span className={[styles['financeMetricIcon'], toneClass].filter(Boolean).join(' ')} aria-hidden="true">{icon}</span>
