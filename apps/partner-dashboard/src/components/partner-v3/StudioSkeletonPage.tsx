...
export async function StudioSkeletonPage({ studio, title, description }: { readonly studio: StudioRole; readonly title: string; readonly description: string }) {
  const shellData = await fixturePartnerDataSource.getStudioShell(studio);

  return <StudioRoutePlaceholder studioLabel={shellData.displayName} title={title} description={description} />;
}
