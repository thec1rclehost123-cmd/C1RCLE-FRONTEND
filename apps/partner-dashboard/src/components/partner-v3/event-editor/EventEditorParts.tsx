...
export function EventPromoterSelector({ data, draft, update }: { readonly data: EventEditorData; readonly draft: EventEditorDraft; readonly update: DraftUpdate }) {
  ...
  const toggle = (id: string) => { update({ selectedPromoterIds: selected.has(id) ? draft.selectedPromoterIds.filter((item) => item !== id) : [...draft.selectedPromoterIds, id] }); };
  return <div className={styles['formColumn']}><section className={styles['card']}><div className={styles['cardTitle']}>Promoters on this event</div><p className={styles['muted']}>Turn on to assign partnered promoters and set how they’re paid.</p><div className={styles['promoterList']}>{data.promoters.map((promoter) => <button className={[styles['promoterRow'], selected.has(promoter.id) ? styles['promoterRowSelected'] : ''].filter(Boolean).join(' ')} key={promoter.id} type="button" onClick={() => { toggle(promoter.id); }}><span className={styles['check']}>{selected.has(promoter.id) ? '✓' : ''}</span><span className={styles['promoterIdentity']}><strong>{promoter.name}</strong><span>{promoter.role}</span></span></button>)}</div></section><section className={styles['card']}><div className={styles['cardTitle']}>Compensation model</div><div className={styles['compGrid']}>{[['standard', 'Global commission', 'One rate across ticket tiers'], ['custom', 'Per-tier commission', 'Different rates by tier'], ['salary', 'Salary', 'Paid outside the platform']].map(([value, label, sub]) => <button className={[styles['compChoice'], draft.compensation === value ? styles['compChoiceSelected'] : ''].filter(Boolean).join(' ')} key={value} type="button" onClick={() => { update({ compensation: value as EventEditorDraft['compensation'] }); }}><strong>{label}</strong><span>{sub}</span></button>)}</div>{draft.compensation === 'salary' ? <div className={[styles['field'], styles['salaryField']].join(' ')}><label htmlFor="salary-notes">Salary notes</label><textarea id="salary-notes" value={draft.salaryNotes} onChange={(event) => { update({ salaryNotes: event.target.value }); }} placeholder="e.g. All ticket sales are covered under promoter salary." /></div> : <div className={styles['rateField']}><input aria-label="Commission rate" inputMode="decimal" value={String(draft.commissionRate)} onChange={(event) => { update({ commissionRate: numberValue(event.target.value) }); }} /><span>% commission</span></div>}</section><section className={styles['card']}><div className={styles['cardTitle']}>Revenue preview</div><div className={styles['reviewRows']}><div className={styles['reviewRow']}><span>Gross ticket value</span><strong>{formatMoney(draft.ticketTiers.reduce((total, tier) => total + tier.price * tier.quantity, 0))}</strong></div><div className={styles['reviewRow']}><span>{draft.compensation === 'salary' ? 'Ticket commission (salaried)' : 'Promoter commission'}</span><strong>{draft.compensation === 'salary' ? '—' : formatMoney(draft.ticketTiers.reduce((total, tier) => total + tier.price * tier.quantity, 0) * draft.commissionRate / 100)}</strong></div></div></section></div>;
}

export function EventReview({ draft, venueName, editMode, errors, studio }: { readonly draft: EventEditorDraft; readonly venueName: string; readonly editMode: boolean; readonly errors: readonly string[]; readonly studio: 'venue' | 'host' }) {
  const soldOut = draft.ticketTiers.reduce((total, tier) => total + tier.price * tier.quantity, 0);
  return <section className={styles['card']}><div className={styles['cardTitle']}>{editMode ? 'Review & save' : studio === 'host' ? 'Review & submit' : 'Review & publish'}</div><div className={styles['reviewRows']}><div className={styles['reviewRow']}><span>Event</span><strong>{draft.name || 'Untitled event'}</strong></div><div className={styles['reviewRow']}><span>Venue</span><strong>{venueName}</strong></div><div className={styles['reviewRow']}><span>Date & time</span><strong>{draft.dateLabel} · {draft.time}</strong></div><div className={styles['reviewRow']}><span>Ticket tiers</span><strong>{draft.ticketTiers.length} tiers · {draft.ticketTiers.reduce((total, tier) => total + tier.quantity, 0)} total</strong></div><div className={styles['reviewRow']}><span>Gross ticket value</span><strong>{formatMoney(soldOut)}</strong></div></div>{errors.length ? <div className={styles['validation']} role="alert"><strong>Finish these items before continuing</strong><ul>{errors.map((error) => <li key={error}>{error}</li>)}</ul></div> : <div className={styles['successNote']}>✓ Everything looks good. {studio === 'host' ? 'The venue will review this request before it can be published.' : 'Guests will see the poster preview exactly as shown.'}</div>}</section>;
}

export function EventPreview({ draft, venueName, onOpen }: { readonly draft: EventEditorDraft; readonly venueName: string; readonly onOpen: () => void }) {
  return <aside className={styles['previewColumn']}><div className={styles['previewLabel']}><i />Live preview · what guests see</div><button className={styles['previewCard']} type="button" onClick={onOpen}><div className={[styles['previewBackground'], gradientClass(draft.artwork.value)].join(' ')} />{draft.artwork.type === 'image' ? <Image fill sizes="380px" className={styles['previewImage']} src={draft.artwork.value} alt="" /> : null}<div className={styles['previewShade']} /><div className={styles['previewContent']}><span className={styles['previewStatus']}><i />Draft</span><div className={styles['previewPrice']}>{formatMoney(draft.ticketTiers[0]?.price ?? 0)}</div><div className={styles['previewName']}>{draft.name.length ? draft.name : 'Your event name'}</div><div className={styles['previewMeta']}>{venueName} · {draft.dateLabel}</div><div className={styles['buyButton']}>Buy tickets</div></div></button><div className={styles['previewHint']}>Click preview to choose Guest portal or Mobile app</div></aside>;
}

export function EventPreviewOverlay({ mode, onPick, onClose }: { readonly mode: 'picker' | 'guest' | 'mobile'; readonly onPick: (mode: 'guest' | 'mobile') => void; readonly onClose: () => void }) {
  return <dialog className={styles['overlay']} open aria-label="Preview destination">{mode === 'picker' ? <div className={styles['previewPicker']}><button type="button" onClick={() => { onPick('guest'); }}>◉ Guest portal</button><button type="button" onClick={() => { onPick('mobile'); }}>▣ Mobile app</button></div> : <div className={styles['overlayStack']}>{mode === 'guest' ? <div className={styles['deviceFrame']}>This is how it will look in the guest portal</div> : <div className={styles['mobileFrame']}><div>This is how it will look in the mobile app</div></div>}<button className={styles['overlayClose']} type="button" onClick={onClose}>← Back to editing</button></div>}</dialog>;
}

export function EventEditorShell({ children, studio }: { readonly children: ReactNode; readonly studio: 'venue' | 'host' }) { return <div className={[styles['page'], studio === 'host' ? styles['hostTheme'] : styles['venueTheme']].join(' ')}>{children}</div>; }

function gradientClass(value: string) { return ({ sunset: styles['gradientSunset'], warehouse: styles['gradientWarehouse'], bollywood: styles['gradientBollywood'], monsoon: styles['gradientMonsoon'] } as Record<string, string | undefined>)[value] ?? styles['posterGradient']; }
function initialsFor(value: string) { return value.split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase(); }
function numberValue(value: string) { const parsed = Number(value.replace(/[^0-9.]/g, '')); return Number.isFinite(parsed) ? parsed : 0; }
