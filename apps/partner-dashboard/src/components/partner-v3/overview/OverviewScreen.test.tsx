...
describe('OverviewScreen', () => {
  it('renders the HTML-reference hierarchy and truthful fixture routes', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Neon Nights: Afrobeats Edition' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Recent activity' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'July 2026' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'Upcoming events' })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: 'My network' })).toBeInTheDocument();
    expect(screen.getByText('Fixture data')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /view guest list/i })).toHaveAttribute('href', '/partner/venue/events/neon-nights-afrobeats/guests');
  ...
  it('keeps Host navigation and partnered-venue availability links role-specific', async () => {
    ...
    expect(screen.getByRole('link', { name: /see all events/i })).toHaveAttribute('href', '/partner/host/calendar');
    expect(screen.getAllByRole('link', { name: /see all$/i }).map((link) => link.getAttribute('href'))).toContain('/partner/host/events');
    expect(screen.getByRole('link', { name: /view all/i })).toHaveAttribute('href', '/partner/host/finance');
  });
});
