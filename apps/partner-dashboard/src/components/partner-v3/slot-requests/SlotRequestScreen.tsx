...
export function SlotRequestScreen({ data, initialView = 'pending', initialRequestId, initialPanel = 'details', initialPreviewMode = 'guest' }: SlotRequestScreenProps) {
  ...
  const visibleRequests = visibleView === 'pending' ? data.requests.filter((request) => request.status === 'pending') : data.requests;
  ...
  return (
    <PageContainer>
      <div className={[styles['page'], data.accent === 'lavender' ? styles['hostTheme'] : ''].filter(Boolean).join(' ')}>
        <div className={styles['topRow']}><Link className={styles['backLink']} href={isIncoming ? '/partner/venue/events' : '/partner/host/events'}><BackIcon size={16} aria-hidden="true" />Back to Events</Link><Button type="button" variant="ghost" onClick={() => { window.location.reload(); }}><RefreshIcon size={15} aria-hidden="true" />Refresh</Button></div>
