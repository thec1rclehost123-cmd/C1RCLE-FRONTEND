...
export function PartnerNotifications({ data }: { readonly data: PartnerNotificationsData }) {
  ...
  return <div className={styles['notificationRoot']} ref={rootRef}>
    <button ref={triggerRef} className={styles['notificationButton']} type="button" aria-label="Notifications" aria-expanded={open} aria-controls="partner-notifications-panel" onClick={() => { setOpen((value) => !value); setRead(true); }}>
      <NotificationIcon size={18} aria-hidden="true" />
      {unreadCount ? <span className={styles['notificationUnreadDot']} aria-label={`${String(unreadCount)} unread notifications`} /> : null}
    </button>
...
function NotificationRow({ notification, read, onClose }: { readonly notification: PartnerNotification; readonly read: boolean; readonly onClose: () => void }) {
  const Icon = notificationIcons[notification.icon];
  const content = <><span className={[styles['notificationIcon'], styles[`notificationIcon${notification.type}`]].join(' ')}><Icon size={16} aria-hidden="true" /></span><span className={styles['notificationCopy']}><span>{notification.description}</span><time>{notification.time}</time></span>{notification.unread && !read ? <i className={styles['notificationDot']} aria-label="Unread" /> : null}</>;
  return notification.href ? <Link className={styles['notificationRow']} href={notification.href} onClick={onClose}>{content}</Link> : <div className={styles['notificationRow']}>{content}</div>;
}
