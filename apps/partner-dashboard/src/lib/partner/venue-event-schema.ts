...
const localEventTimestamp = (date: string, time: string, timezone: keyof typeof timezoneOffsets): number => {
...
export const venueEventDraftSchema = z.object({
  ...
  if (end <= start) context.addIssue({ code: 'custom', path: ['endTime'], message: 'End time must be after start time.' });
  if (start <= Date.now()) context.addIssue({ code: 'custom', path: ['eventDate'], message: 'Event must start in the future.' });
  const tierInventory = draft.ticketTiers.reduce((total, tier) => total + tier.inventory, 0);
  if (tierInventory > draft.capacity) context.addIssue({ code: 'custom', path: ['ticketTiers'], message: 'Ticket inventory cannot exceed venue capacity.' });
  draft.ticketTiers.forEach((tier, index) => {
    const saleStart = Date.parse(tier.saleStartsAt);
    const saleEnd = Date.parse(tier.saleEndsAt);
    if (saleEnd <= saleStart) context.addIssue({ code: 'custom', path: ['ticketTiers', index, 'saleEndsAt'], message: 'Ticket sale end must be after sale start.' });
    if (saleEnd > start) context.addIssue({ code: 'custom', path: ['ticketTiers', index, 'saleEndsAt'], message: 'Ticket sales must close before the event starts.' });
  });
});
