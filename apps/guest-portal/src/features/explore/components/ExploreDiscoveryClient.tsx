...
export function ExploreDiscoveryClient({
  ...
  return (
    <section
      ...
      className="relative z-10 -mt-20 bg-black px-4 pb-12 pt-28 sm:px-6 sm:pt-32 lg:px-8"
    >
      <div className="mx-auto max-w-[1540px]">
        ...
        <div className="relative z-20 mb-12 rounded-[2rem] border border-white/12 bg-white/[0.045] p-2 shadow-[0_24px_80px_rgba(0,0,0,0.38)] backdrop-blur-2xl lg:flex lg:items-stretch lg:rounded-full">
          <label className="group flex min-h-16 min-w-0 flex-1 items-center gap-4 rounded-[1.45rem] bg-white/[0.055] px-5 transition-colors focus-within:bg-white/[0.085] lg:rounded-full lg:px-7">
            <SearchIcon />
            <span className="flex min-w-0 flex-1 flex-col gap-1">
              <span className="text-[9px] font-black uppercase tracking-[0.22em] text-white/42">
                Search events
              </span>
              <input
                type="search"
                ...
                className="min-h-7 min-w-0 bg-transparent text-sm font-bold text-white outline-none placeholder:text-white/28 sm:text-base"
              />
            </span>
          </label>

          <div className="mt-2 grid grid-cols-3 divide-x divide-white/10 lg:mt-0 lg:min-w-[500px]">
            <ExploreSelect
              label="Sort"
              value={sort}
              onChange={(value) => {
...
function ExploreSelect({
  ...
  return (
    <label className="flex min-w-0 flex-col justify-center gap-1 px-3 py-3 sm:px-5">
      ...
      <select
        ...
        className="min-h-7 max-w-full cursor-pointer bg-transparent text-[11px] font-bold text-white outline-none sm:text-xs"
      >
        {children}
      </select>
    </label>
...
function SearchIcon() {
  return (
    <svg
      aria-hidden="true"
      viewBox="0 0 24 24"
      className="size-5 shrink-0 fill-none stroke-[#FF6842] stroke-2 sm:size-6"
    >
