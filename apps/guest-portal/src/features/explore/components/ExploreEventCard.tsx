  ...
  return (
    <article className="group h-full">
      <Link
        ...
        className="relative block aspect-[4/5] min-h-[26rem] overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/5 shadow-lg transition-transform duration-300 hover:-translate-y-1 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white motion-reduce:transition-none"
      >
        <Image
          src={event.image}
        ...
        <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
          ...
          <h3 className="mt-2 text-3xl font-black uppercase leading-none tracking-[-0.04em] text-white">
            {event.title}
          </h3>
          <p className="mt-2 text-sm font-medium text-white/65">
            {event.venue}, {event.city}
          </p>
          <span className="mt-5 inline-flex min-h-11 items-center rounded-full bg-white px-5 py-3 text-[10px] font-black uppercase tracking-[0.18em] text-black">
            Book tickets
          </span>
        </div>
      </Link>
